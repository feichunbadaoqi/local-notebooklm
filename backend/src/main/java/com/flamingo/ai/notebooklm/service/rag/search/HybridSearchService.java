package com.flamingo.ai.notebooklm.service.rag.search;
...
public class HybridSearchService {
  ...
  public String buildContext(List<DocumentChunk> chunks) {
    ...
    // Map unique documentId to source number (1, 2, 3...)
    Map<UUID, Integer> documentIdToSourceNumber = new HashMap<>();
    int sourceCounter = 1;
    for (DocumentChunk chunk : chunks) {
      UUID docId = chunk.getDocumentId();
      if (docId != null && !documentIdToSourceNumber.containsKey(docId)) {
        documentIdToSourceNumber.put(docId, sourceCounter++);
      }
    }
    ...
    for (int i = 0; i < chunks.size(); i++) {
      ...
      // Get source number based on documentId (same document = same number)
      int sourceNumber =
          chunk.getDocumentId() != null
              ? documentIdToSourceNumber.getOrDefault(chunk.getDocumentId(), i + 1)
              : i + 1;

      // Build source header with metadata
      StringBuilder sourceHeader = new StringBuilder();
      sourceHeader.append(String.format("[Source %d: %s", sourceNumber, chunk.getFileName()));
